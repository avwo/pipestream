...
function PipeStream(options) {
  ...
  init(this);
  this._pipeError = options && options.pipeError;
...
proto.src = function(src, pipeOpts) {
	var self = this;
	self._originalStream && self._originalStream.emit('src', src);
...
function pipeStream(pipes, i, callback) {
	...
	if (typeof pipe == 'function') {
		...
		src.once('pipeEnd', pipeEnd);
		src.pipe(pt);
	} else {
		pipeEnd(src);
	}
	
	function pipeEnd(src) {
...
PipeStream.wrap = function(stream, dest, pipeOpts, options) {
	return new PipeStream(options).wrapStream(stream, dest, pipeOpts);
};

PipeStream.wrapSrc = function(stream, pipeOpts, options) {
	
	return PipeStream.wrap(stream, false, pipeOpts, options);
};

PipeStream.wrapDest = function(stream, pipeOpts, options) {
	
	return PipeStream.wrap(stream, true, pipeOpts, options);
};
